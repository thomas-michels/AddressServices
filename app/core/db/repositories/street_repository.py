...
class StreetRepository(Repository):
    ...
    async def insert(self, street: Street) -> StreetInDB:
        try:
            query = '''
             INSERT INTO development.streets(
            ...
            raw_street = await self.conn.execute(sql_statement=query, values=street.model_dump())
    ...
    async def select_by_name(self, name: str) -> StreetInDB:
        try:
            ...
            raw_street = await self.conn.execute(sql_statement=query, values={"name": name})
